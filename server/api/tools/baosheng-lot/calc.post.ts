...
function localizeFortune(fortune: FortuneLot, locale: string): DrawALotCalcResult['fortune'] {
  ...
  return {
    ...
    categoryJudgments: fortune.categoryJudgments?.map(item => ({
      label: item.label[loc] ?? item.label['zh-CN']!,
      value: item.value[loc] ?? item.value['zh-CN']!,
    })),
  }
}
...
export default defineEventHandler(async (event) => {
  ...
  if (!LOT_TYPES.some(type => type.id === 'baoshengdadi')) {
    throw createError({ statusCode: 400, statusMessage: 'Unsupported lot type: baoshengdadi' })
  }

  const { lotType, fortune } = drawFortune('baoshengdadi', locale)
  return {
