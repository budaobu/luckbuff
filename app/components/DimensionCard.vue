<template>
  <div class="group relative rounded-2xl bg-[var(--surface-input)] backdrop-blur-sm overflow-hidden transition-all duration-500 hover:bg-[var(--surface-card-hover)]">
    ...
    <div
      ...
      :style="{ backgroundImage: `linear-gradient(to right, transparent, ${accentColor}66, transparent)` }"
    />
    ...
    <div class="relative z-10 p-6">
      ...
      <div class="flex items-center gap-3 mb-4">
        <div
          class="w-10 h-10 rounded-xl flex items-center justify-center transition-transform duration-500 group-hover:scale-110"
...
import type { AiDimensionCard } from '~/types/bazi'

interface Props {
  title: string
  icon: string
  accentColor: string
  data: AiDimensionCard
  extraField?: {
    label: string
    value: string
    icon: string
