...
export default defineEventHandler(async (event) => {
  ...
  if (!LOT_TYPES.some(type => type.id === 'tudigong')) {
    throw createError({ statusCode: 400, statusMessage: 'Unsupported lot type: tudigong' })
  }

  const { lotType, fortune } = drawFortune('tudigong', locale)
  return {
