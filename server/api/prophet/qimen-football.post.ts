import { createQimenFootballPrediction } from '~~/server/utils/football/prediction'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    return await createQimenFootballPrediction({
    ...
    throw createError({
      statusCode: 503,
      statusMessage: error instanceof Error ? error.message : 'Qimen engine unavailable',
    })
  }
})
