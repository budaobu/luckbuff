  ...
  <div class="relative z-10 mx-auto max-w-3xl px-5 py-12">
    <div v-if="phase === 'form'">
      <header class="mb-8">
        ...
        <h1 class="font-serif text-3xl font-bold tracking-tight text-[var(--text-primary)]">
          {{ $t('footballPrediction.liurenTitle') }}
        </h1>
        <p class="mt-2 text-sm text-[var(--text-faint)]">
          {{ $t('footballPrediction.liurenSubtitle') }}
        </p>
      </header>
      ...
      <form class="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-dropdown)] p-5 sm:p-6" @submit.prevent="submit">
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <label class="block">
            <span class="mb-2 block text-sm text-[var(--text-muted)]">{{ $t('footballPrediction.form.birthYear') }}</span>
            <UInput
              ...
              min="1900"
              max="2100"
              :placeholder="$t('footballPrediction.form.birthYearPlaceholder')"
              class="w-full"
            />
          </label>
        </div>
        ...
        >
          <template #leading>
            <UIcon name="i-heroicons-cube-transparent" class="h-5 w-5" />
          </template>
          {{ $t('footballPrediction.form.liurenSubmit') }}
        </UButton>
      </form>
    </div>
    ...
    <div v-else-if="result" class="space-y-6">
      <FootballPredictionPoster
        ref="posterRef"
        :result="result"
        :method-label="$t('footballPrediction.methods.liuren')"
      />
      ...
      <div class="flex flex-wrap justify-center gap-3">
        <AppShareButton
          ...
          :summary="`${result.liuren?.dayGanzhi || ''}${t('footballPrediction.poster.dayUnit')}${result.liuren?.shichen || ''}${t('footballPrediction.poster.hourUnit')} · ${result.prediction.primaryScore.home}-${result.prediction.primaryScore.away}`"
          :share-target="posterTarget"
          :filename="`liuren-football-${result.generatedAt.slice(0, 10)}.png`"
        />
...
const birthYear = computed(() => Number.parseInt(form.birthYear, 10))
...
useSeoMeta({
  ...
  keywords: () => t('footballPrediction.liurenKeywords'),
