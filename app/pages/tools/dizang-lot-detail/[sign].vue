  ...
  <div v-if="lot" class="relative overflow-hidden">
    ...
    <article class="relative z-10 mx-auto max-w-3xl px-6 py-12">
      ...
      <section class="mb-5 rounded-2xl border border-[var(--border-light)] bg-[var(--surface-card)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('dizangLotDetail.poem') }}</h2>
      ...
      <section class="mb-5 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-dropdown)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('dizangLotDetail.sacredMeaning') }}</h2>
      ...
      <section class="mb-5 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('dizangLotDetail.signHint') }}</h2>
      ...
      <section class="mb-7 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('dizangLotDetail.advice') }}</h2>
        <p class="whitespace-pre-line text-sm leading-7 text-[var(--text-body)]">{{ lot.fortune.advice }}</p>
      </section>

      <nav class="mb-8 grid grid-cols-2 gap-3" :aria-label="$t('dizangLotDetail.navigation')">
        <NuxtLink
          v-if="lot.fortune.number > 1"
          :to="localePath(`/tools/dizang-lots/${lot.fortune.number - 1}`)"
          class="rounded-xl border border-[var(--border-light)] bg-[var(--surface-card)] p-4 transition-colors hover:border-[var(--border-medium)]"
        >
          <span class="text-xs text-[var(--text-faint)]">{{ $t('dizangLotDetail.previous') }}</span>
          <span class="mt-1 block text-sm font-medium">{{ $t('dizangLotDetail.lotNo', { n: lot.fortune.number - 1 }) }}</span>
        </NuxtLink>
        <div v-else />
        <NuxtLink
        ...
        >
          <span class="text-xs text-[var(--text-faint)]">{{ $t('dizangLotDetail.next') }}</span>
          <span class="mt-1 block text-sm font-medium">{{ $t('dizangLotDetail.lotNo', { n: lot.fortune.number + 1 }) }}</span>
        </NuxtLink>
      </nav>
...
const pageUrl = useLocalizedSeoUrl(() => `/tools/dizang-lots/${sign.value}`)
...
useSeoMeta({
  title: () => t('dizangLotDetail.seoTitle', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
  description: () => t('dizangLotDetail.seoDescription', {
    n: lot.value!.fortune.number,
    title: lot.value!.fortune.title,
    meaning: lot.value!.fortune.explanation,
  }),
  keywords: () => t('dizangLotDetail.seoKeywords', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
  ogTitle: () => t('dizangLotDetail.seoTitle', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
  ogDescription: () => t('dizangLotDetail.seoDescription', {
    n: lot.value!.fortune.number,
    title: lot.value!.fortune.title,
    meaning: lot.value!.fortune.explanation,
  }),
...
useHead(() => ({
  script: [{
    ...
    innerHTML: JSON.stringify({
      ...
      '@type': 'Article',
      headline: t('dizangLotDetail.seoTitle', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
      description: t('dizangLotDetail.seoDescription', {
        n: lot.value!.fortune.number,
        title: lot.value!.fortune.title,
        meaning: lot.value!.fortune.explanation,
