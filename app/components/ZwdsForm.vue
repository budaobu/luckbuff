<template>
  <div class="space-y-5">
    ...
    <div v-if="profiles.length > 0" class="space-y-2">
      <label class="text-xs font-medium text-[var(--text-muted)]">{{ $t('zwdsForm.selectProfile') }}</label>
    ...
    <div v-else class="rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-card)] px-4 py-3">
      <p class="text-sm text-[var(--text-faint)]">
        {{ $t('zwdsForm.noProfiles') }}<NuxtLink :to="localePath('/settings')" class="text-[var(--accent)] hover:underline">{{ $t('zwdsForm.goSettings') }}</NuxtLink>{{ $t('zwdsForm.createSuffix') }}
      </p>
    </div>
    ...
    <div class="space-y-1.5">
      ...
      />
      <p class="text-xs text-[var(--text-placeholder)]">{{ $t('zwdsForm.hourWarning') }}</p>
    </div>
...
const { dateToGanZhi } = useZwdsCalc()
...
function selectProfile(profile: UserProfile, autoSubmit = false) {
  ...
  onBirthDateChange()
  // 回访流程：选择完整档案后自动排盘
  if (autoSubmit && form.gender && form.birthDate) {
    nextTick(() => emit('submit', { ...form }))
  }
}
