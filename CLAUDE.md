...
## 皇极经世值年卦（huangji-zhiniangua）纪元锚点

- 锚点：帝尧元年 = 甲辰 = 公元前2357年（天文纪年 -2356），该年值年卦 = 雷火丰
  （六十卦序列 index 12）。两者均经两个独立资料源交叉验证，来源清单、
  桥接演算与流派分歧（祝泌层叠法推大有，本工具不采用）的完整留痕见
  `docs/huangji-zhiniangua-algorithm.md`，改动算法前先读它。
- 六十卦循环顺序、积年换算、世内每5年一爻的取法均按该文档实现，
  不要改用其他流派的顺序或取法。
