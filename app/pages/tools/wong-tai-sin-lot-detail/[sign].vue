    ...
    <article class="relative z-10 mx-auto max-w-3xl px-6 py-12">
      ...
      <header class="mb-8">
        <p class="mb-2 text-xs uppercase tracking-[0.2em] text-[var(--accent-muted)]">
          Wong Tai Sin Oracle No. {{ lot.fortune.number }}
        </p>
        <h1 class="font-serif text-3xl font-bold tracking-tight text-[var(--text-primary)] md:text-4xl">
          {{ $t('wongTaiSinLotDetail.title', { n: lot.fortune.number }) }}
        </h1>
      ...
      <section class="mb-5 rounded-2xl border border-[var(--border-light)] bg-[var(--surface-card)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('wongTaiSinLotDetail.poem') }}</h2>
        <p class="whitespace-pre-line text-base leading-8 text-[var(--text-body)]">{{ lot.fortune.poem }}</p>
      </section>

      <section v-if="lot.fortune.allusion" class="mb-5 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-dropdown)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('wongTaiSinLotDetail.allusion') }}</h2>
        <p class="whitespace-pre-line text-sm leading-7 text-[var(--text-body)]">{{ lot.fortune.allusion }}</p>
      </section>

      <section class="mb-5 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('wongTaiSinLotDetail.explanation') }}</h2>
      ...
      <section class="mb-7 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('wongTaiSinLotDetail.advice') }}</h2>
        <p class="whitespace-pre-line text-sm leading-7 text-[var(--text-body)]">{{ lot.fortune.advice }}</p>
      </section>

      <nav class="mb-8 grid grid-cols-2 gap-3" :aria-label="$t('wongTaiSinLotDetail.navigation')">
        <NuxtLink
          v-if="lot.fortune.number > 1"
          :to="localePath(`/tools/wong-tai-sin-lots/${lot.fortune.number - 1}`)"
          class="rounded-xl border border-[var(--border-light)] bg-[var(--surface-card)] p-4 transition-colors hover:border-[var(--border-medium)]"
        >
          <span class="text-xs text-[var(--text-faint)]">{{ $t('wongTaiSinLotDetail.previous') }}</span>
          <span class="mt-1 block text-sm font-medium">{{ $t('wongTaiSinLotDetail.lotNo', { n: lot.fortune.number - 1 }) }}</span>
        </NuxtLink>
        <div v-else />
        <NuxtLink
        ...
        >
          <span class="text-xs text-[var(--text-faint)]">{{ $t('wongTaiSinLotDetail.next') }}</span>
          <span class="mt-1 block text-sm font-medium">{{ $t('wongTaiSinLotDetail.lotNo', { n: lot.fortune.number + 1 }) }}</span>
        </NuxtLink>
      </nav>
...
definePageMeta({
  path: '/tools/wong-tai-sin-lots/:sign(\\d+)',
})
...
const pageUrl = useLocalizedSeoUrl(() => `/tools/wong-tai-sin-lots/${sign.value}`)
...
useSeoMeta({
  title: () => t('wongTaiSinLotDetail.seoTitle', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
  description: () => t('wongTaiSinLotDetail.seoDescription', {
    n: lot.value!.fortune.number,
    title: lot.value!.fortune.title,
    meaning: lot.value!.fortune.explanation,
  }),
  keywords: () => t('wongTaiSinLotDetail.seoKeywords', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
  ogTitle: () => t('wongTaiSinLotDetail.seoTitle', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
  ogDescription: () => t('wongTaiSinLotDetail.seoDescription', {
    n: lot.value!.fortune.number,
    title: lot.value!.fortune.title,
    meaning: lot.value!.fortune.explanation,
  }),
...
useHead(() => ({
  script: [{
    ...
    innerHTML: JSON.stringify({
      ...
      '@type': 'Article',
      headline: t('wongTaiSinLotDetail.seoTitle', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
      description: t('wongTaiSinLotDetail.seoDescription', {
        n: lot.value!.fortune.number,
        title: lot.value!.fortune.title,
        meaning: lot.value!.fortune.explanation,
