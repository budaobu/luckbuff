...
import { getParentingStyleProfile, getDimensionDistribution } from '~/data/parenting-style'
...
export default defineEventHandler(async (event): Promise<ParentingStyleCalcResult> => {
  ...
  const style = getParentingStyleProfile(shishenCounts)
  const dimensionDistribution = getDimensionDistribution(shishenCounts)
