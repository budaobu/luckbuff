  ...
  <div class="relative z-10 mx-auto max-w-3xl px-5 py-12">
    <div v-if="phase === 'form'">
      <header class="mb-8">
        ...
        <h1 class="font-serif text-3xl font-bold tracking-tight text-[var(--text-primary)]">
          {{ $t('footballPrediction.qimenTitle') }}
        </h1>
        <p class="mt-2 text-sm text-[var(--text-faint)]">
          {{ $t('footballPrediction.qimenSubtitle') }}
        </p>
      </header>
      ...
      <form class="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-dropdown)] p-5 sm:p-6" @submit.prevent="submit">
        ...
        >
          ...
          {{ $t('footballPrediction.form.qimenSubmit') }}
        </UButton>
      </form>
    </div>
    ...
    <div v-else-if="result" class="space-y-6">
      <FootballPredictionPoster
        ref="posterRef"
        :result="result"
        :method-label="$t('footballPrediction.methods.qimen')"
      />
      ...
      <div class="flex flex-wrap justify-center gap-3">
        <AppShareButton
          ...
          :summary="`${result.qimen?.title || ''} · ${result.prediction.primaryScore.home}-${result.prediction.primaryScore.away}`"
          :share-target="posterTarget"
          :filename="`qimen-football-${result.generatedAt.slice(0, 10)}.png`"
        />
...
async function submit() {
  ...
  try {
    ...
    result.value = await plainFetch<FootballPredictionResult>('/api/prophet/qimen-football', {
      method: 'POST',
      body: payload,
    })
    phase.value = 'result'
...
useSeoMeta({
  ...
  keywords: () => t('footballPrediction.qimenKeywords'),
