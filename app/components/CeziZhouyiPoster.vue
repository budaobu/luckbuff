  ...
  <div class="czp">
    <div class="czp-sheet">
      ...
      <header class="czp-head">
        <p class="czp-kicker">{{ $t('ceziZhouyi.poster.kicker') }}</p>
        <h2 class="czp-title">{{ $t('ceziZhouyi.poster.title') }}</h2>
        <p class="czp-serial">{{ $t('ceziZhouyi.poster.serial') }}</p>
        <!-- AI 点题：这个字问这件事是什么兆头 -->
        <p class="czp-overview" :class="{ 'czp-pending': !aiParsed.overview }">
          {{ aiParsed.overview || $t('ceziZhouyi.poster.overviewPending') }}
        </p>
      </header>

      <!-- ============ 所问之事：签文式横批 ============ -->
      <section class="czp-question">
        <span class="czp-question-flag">{{ $t('ceziZhouyi.poster.questionFlag') }}</span>
      ...
      <!-- ============ 推演批注：笔画起卦 → 上下卦 → 动爻 → 本卦，朱批小字行 ============ -->
      <div class="czp-derive">
        <p class="czp-derive-line">
          <span class="czp-derive-flag">{{ $t('ceziZhouyi.poster.strokeFlag') }}</span>
          <span class="czp-derive-text">{{ strokeLine }}</span>
        </p>
        <p class="czp-derive-line">
          <span class="czp-derive-flag">{{ $t('ceziZhouyi.poster.trigramFlag') }}</span>
          <span class="czp-derive-text">{{ trigramLine }}</span>
        </p>
        <p class="czp-derive-line">
          <span class="czp-derive-flag">{{ $t('ceziZhouyi.poster.movingFlag') }}</span>
          <span class="czp-derive-text">{{ movingLine }}</span>
        </p>
      </div>

      <!-- ============ 卦象白话 ============ -->
      <p class="czp-hex" :class="{ 'czp-pending': !aiParsed.hex }">
        <span class="czp-hex-label">{{ $t('ceziZhouyi.poster.hexLabel') }}</span>
        <span class="czp-hex-name">{{ hexagramName }}</span>
        <span class="czp-hex-text">{{ aiParsed.hex || $t('ceziZhouyi.poster.fieldPending') }}</span>
      </p>

      <!-- ============ 竖排断语：字旁签文区 ============ -->
      <section class="czp-verdict">
        <div class="czp-verdict-scroll">
          <span class="czp-verdict-flag">{{ $t('ceziZhouyi.poster.verdictFlag') }}</span>
          <p class="czp-verdict-text" :class="{ 'czp-pending': !aiParsed.verdict }">
            {{ aiParsed.verdict || $t('ceziZhouyi.poster.verdictPending') }}
          </p>
        </div>
      </section>
      ...
      <!-- ============ 隐士收口提点 ============ -->
      <p class="czp-note">
        <span class="czp-note-label">{{ $t('ceziZhouyi.poster.noteLabel') }}</span>
        <span :class="{ 'czp-pending': !aiParsed.tip }">
          {{ aiParsed.tip || $t('ceziZhouyi.poster.notePending') }}
        </span>
      </p>
      ...
      <footer class="czp-foot">
        <div class="czp-foot-row">
          <div class="czp-sign">
            ...
            <p class="czp-sign-line czp-sign-who">{{ $t('ceziZhouyi.poster.signWho') }}</p>
          </div>
          ...
          <div class="czp-qr" aria-hidden="true">
            ...
            <span v-else class="czp-qr-inner">{{ $t('ceziZhouyi.poster.qrHint') }}</span>
          </div>
        </div>
      </footer>
    </div>
...
const mainChar = computed(() => analysis.value.char ?? props.result.input.char ?? '')

const mainCharMeta = computed(() => {
  const a = analysis.value
  return t('ceziZhouyi.poster.charMeta', {
    pinyin: a.pinyin,
    strokes: a.strokeCount,
    wuxing: a.wuxing,
  })
})

const questionText = computed(
  () => props.result.input.question || t('ceziZhouyi.noQuestion'),
)

const hexagramName = computed(() => {
  const h = props.result.hexagram
  return `${h.hexagramName} ${h.hexagram}`
})

/* ---------- 推演批注：笔画起卦 / 上下卦 / 动爻，朱批小字 ---------- */

const strokeLine = computed(() => {
  const a = analysis.value
  return t('ceziZhouyi.poster.strokeLine', {
    char: a.char,
    strokes: a.strokeCount,
...
const trigramLine = computed(() => {
  const h = props.result.hexagram
  return t('ceziZhouyi.poster.trigramLine', {
    upperSymbol: h.upperTrigram,
    upperName: h.upperTrigramName,
    lowerSymbol: h.lowerTrigram,
    lowerName: h.lowerTrigramName,
  })
})

const movingLine = computed(() => {
  const h = props.result.hexagram
  return t('ceziZhouyi.poster.movingLineText', {
    line: h.movingLine,
    hexagram: h.hexagramName,
  })
})
...
const charBreakdown = computed(() => {
  const a = analysis.value
  const parts = a.components && a.components.length > 0 ? a.components : [a.char]
  return parts.slice(0, 4).map(part => ({
    part,
    info: `${a.radical} · ${a.wuxing}`,
  }))
})
...
const sealText = computed(() =>
  t('ceziZhouyi.poster.seal').replace(/\\n/g, '\n'),
)
...
const qrSvg = ref('')
onMounted(async () => {
  const url = `${window.location.origin}/tools/cezi-zhouyi`
