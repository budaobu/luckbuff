...
// Runtime view counters. Lives next to the articles but is excluded from
// deploy rsync (like .backups/) so the server-side counts never get
// overwritten by a stale local copy.
const VIEWS_FILE = resolve(process.cwd(), 'content', 'insights', '.views.json')
...
type ViewsData = Record<string, ArticleViews>
...
export function recordInsightView(slug: string): number {
  const data = load()
  const entry = (data[slug] ??= { total: 0, daily: {} })
  const now = new Date()
  entry.total += 1
...
export interface InsightViewStats {
  last7Total: number
  last30Total: number
...
export function getInsightViewStats(): InsightViewStats {
  const data = load()
...
export function getInsightViewTotal(slug: string): number {
  return load()[slug]?.total ?? 0
}
