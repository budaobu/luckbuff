<template>
  <div v-if="lot" class="relative overflow-hidden">
    ...
    <article class="relative z-10 mx-auto max-w-3xl px-6 py-12">
      ...
      <header class="mb-8">
        <p class="mb-2 text-xs uppercase tracking-[0.2em] text-[var(--accent-muted)]">Guandi Oracle No. {{ lot.fortune.number }}</p>
        <h1 class="font-serif text-3xl font-bold tracking-tight text-[var(--text-primary)] md:text-4xl">
          {{ $t('guandiLotDetail.title', { n: lot.fortune.number }) }}
        </h1>
      ...
      <section class="mb-5 rounded-2xl border border-[var(--border-light)] bg-[var(--surface-card)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('guandiLotDetail.poem') }}</h2>
      ...
      <section class="mb-5 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-dropdown)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('guandiLotDetail.explanation') }}</h2>
      ...
      <section class="mb-7 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5">
        <h2 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">{{ $t('guandiLotDetail.advice') }}</h2>
        <p class="whitespace-pre-line text-sm leading-7 text-[var(--text-body)]">{{ lot.fortune.advice }}</p>
      </section>

      <nav class="mb-8 grid grid-cols-2 gap-3" :aria-label="$t('guandiLotDetail.navigation')">
        <NuxtLink
          v-if="lot.fortune.number > 1"
          :to="localePath(`/tools/guandi-lots/${lot.fortune.number - 1}`)"
          class="rounded-xl border border-[var(--border-light)] bg-[var(--surface-card)] p-4 transition-colors hover:border-[var(--border-medium)]"
        >
          <span class="text-xs text-[var(--text-faint)]">{{ $t('guandiLotDetail.previous') }}</span>
          <span class="mt-1 block text-sm font-medium">{{ $t('guandiLotDetail.lotNo', { n: lot.fortune.number - 1 }) }}</span>
        </NuxtLink>
        <div v-else />
        <NuxtLink
        ...
        >
          <span class="text-xs text-[var(--text-faint)]">{{ $t('guandiLotDetail.next') }}</span>
          <span class="mt-1 block text-sm font-medium">{{ $t('guandiLotDetail.lotNo', { n: lot.fortune.number + 1 }) }}</span>
        </NuxtLink>
      </nav>
...
const pageUrl = useLocalizedSeoUrl(() => `/tools/guandi-lots/${sign.value}`)
...
useSeoMeta({
  title: () => t('guandiLotDetail.seoTitle', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
  description: () => t('guandiLotDetail.seoDescription', {
    n: lot.value!.fortune.number,
    title: lot.value!.fortune.title,
    meaning: lot.value!.fortune.explanation,
  }),
  keywords: () => t('guandiLotDetail.seoKeywords', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
  ogTitle: () => t('guandiLotDetail.seoTitle', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
  ogDescription: () => t('guandiLotDetail.seoDescription', {
    n: lot.value!.fortune.number,
    title: lot.value!.fortune.title,
    meaning: lot.value!.fortune.explanation,
  }),
...
useHead(() => ({
  script: [{
    ...
    innerHTML: JSON.stringify({
      ...
      '@type': 'Article',
      headline: t('guandiLotDetail.seoTitle', { n: lot.value!.fortune.number, title: lot.value!.fortune.title }),
      description: t('guandiLotDetail.seoDescription', {
        n: lot.value!.fortune.number,
        title: lot.value!.fortune.title,
        meaning: lot.value!.fortune.explanation,
