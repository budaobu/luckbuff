...
// 九星阴阳属性（按需求硬编码）
const STAR_YIN: Record<number, boolean> = {
  1: false, // 一白（阳）
  2: true,  // 二黑（阴）
  3: false, // 三碧（阳）
  4: true,  // 四绿（阴）
...
function findMountain(deg: number): Mountain | null {
...
function getPeriod(year: number): { number: number; startYear: number; endYear: number } {
  // 下元七运 1984–2003、下元八运 2004–2023、下运九运 2024–2043
...
function flyBackward(center: number): Record<number, number> {
  const chart: Record<number, number> = {}
  let star = center
...
export default defineEventHandler(async (event) => {
  ...
  if (body == null || typeof body.direction !== 'number' || typeof body.year !== 'number') {
    throw createError({ statusCode: 400, statusMessage: 'Missing or invalid direction/year' })
  }
  ...
  const facing = findMountain(direction)
  if (!facing) {
    throw createError({ statusCode: 400, statusMessage: 'Unable to map direction to 24 mountains' })
  }

  const sittingDeg = normalizeDegree(direction + 180)
  const sitting = findMountain(sittingDeg)
  if (!sitting) {
    throw createError({ statusCode: 400, statusMessage: 'Unable to map sitting direction to 24 mountains' })
  }
  ...
  const pattern = determinePattern(period.number, sitting, facing, mountainChart, facingChart)
  const warning = findBoundaryWarning(direction)
