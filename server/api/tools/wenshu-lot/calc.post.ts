...
export default defineEventHandler(async (event) => {
  ...
  if (!LOT_TYPES.some(type => type.id === 'wenshu')) {
    throw createError({ statusCode: 400, statusMessage: 'Unsupported lot type: wenshu' })
  }

  const { lotType, fortune } = drawFortune('wenshu', locale)
  return {
